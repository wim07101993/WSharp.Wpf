...
namespace WSharp.Wpf.Controls
{
    ...
    internal class Screen
    {
        ...
        // This identifier is just for us, so that we don't try to call the multimon functions if we
        // just need the primary monitor... this is safer for non-multimon OSes.
        ...
        private Screen(IntPtr monitor)
        {
            ...
            {
                // MultiMonitor System We call the 'A' version of GetMonitorInfoA() because the 'W'
                // version just never fills out the struct properly on Win2K.
        ...
        /// <summary>Gets an array of all of the displays on the system.</summary>
            ...
            get
            {
                if (screens == null)
                {
                    if (multiMonitorSupport)
                    {
                        ...
                        _ = NativeMethods.EnumDisplayMonitors(NativeMethods.NullHandleRef, null, proc, IntPtr.Zero);
                        ...
                        {
                            screens = new[] { new Screen((IntPtr)PRIMARY_MONITOR) };
                        }
                    }
                    else
                    {
                    ...
                    // Now that we have our screens, attach a display setting changed event so that
                    // we know when to invalidate them.
        ...
        /// <summary>Gets the working area of the screen.</summary>
        public Rect WorkingArea
        {
            get
            {
                ...
                {
                    _ = Interlocked.Exchange(ref _currentDesktopChangedCount, DesktopChangedCount);
                    ...
                    {
                        // MultiMonitor System We call the 'A' version of GetMonitorInfoA() because
                        // the 'W' version just never fills out the struct properly on Win2K.
                        var info = new NativeMethods.MONITORINFOEX();
                        _ = NativeMethods.GetMonitorInfo(new HandleRef(null, _hmonitor), info);
        ...
        /// <summary>
        ///     Screen instances call this property to determine if their WorkingArea cache needs to
        ///     be invalidated.
        /// </summary>
        ...
        /// <summary>
        ///     Retrieves a <see cref="Screen"/> for the monitor that contains the largest region of
        ///     the Rect.
        /// </summary>
        ...
        /// <summary>
        ///     Called by the SystemEvents class when our display settings are changing. We cache
        ///     screen information and at this point we must invalidate our cache.
        /// </summary>
        private static void OnDisplaySettingsChanging(object sender, EventArgs e)
        {
            // Now that we've responded to this event, we don't need it again until someone
            // re-queries. We will re-add the event at that time.
            SystemEvents.DisplaySettingsChanging -= OnDisplaySettingsChanging;
        ...
        /// <summary>
        ///     Called by the SystemEvents class when our display settings have changed. Here, we
        ///     increment a static counter that Screen instances can check against to invalidate
        ///     their cache.
        /// </summary>
        private static void OnUserPreferenceChanged(object sender, UserPreferenceChangedEventArgs e)
        {
            if (e.Category == UserPreferenceCategory.Desktop)
                _ = Interlocked.Increment(ref desktopChangedCount);
        }
