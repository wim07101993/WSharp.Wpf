...
namespace WSharp.Wpf
{
    public class SnackbarMessageQueue : ISnackbarMessageQueue, IDisposable
    {
        ...
        private class MouseNotOverManagedWaitHandle : IDisposable
        {
            ...
            private void UiElementOnMouseLeave(object sender, MouseEventArgs mouseEventArgs)
            {
                _ = Task.Factory.StartNew(() =>
                {
                    try
                    {
                        _ = _disposedWaitHandle.WaitOne(TimeSpan.FromSeconds(2));
                    }
                ...
                WaitHandle ceaseWaitHandle)
            {
                ...
                _ = Task.Factory.StartNew(() =>
                {
                    ...
                    if (DateTime.Now >= _completionTime)
                        _ = signalWhenDurationPassedWaitHandle.Set();
                });
            }
        ...
        #endregion private class DurationMonitor
        ...
        public SnackbarMessageQueue(TimeSpan messageDuration)
        {
            ...
            _ = Task.Factory.StartNew(PumpAsync);
        }
        ...
        internal Action Pair(Snackbar snackbar)
        {
            ...
            _ = _pairedSnackbars.Add(snackbar);
        ...
        internal Action Pause()
        {
            ...
            return () =>
            {
                if (Interlocked.Decrement(ref _pauseCounter) == 0)
                    _ = _pausedEvent.Reset();
            };
        }

        /// <summary>
        ///     Gets or sets a value that indicates whether this message queue displays messages
        ///     without discarding duplicates. True to show every message even if there are duplicates.
        /// </summary>
        ...
        public void Enqueue(object content, object actionContent, Action actionHandler) => Enqueue(content, actionContent, actionHandler, false);
            ...
            TimeSpan? durationOverride = null)
        {
            ...
            {
                var name = actionContent != null
                    ? nameof(actionContent)
                    : nameof(actionHandler);
                throw new ArgumentException("All action arguments must be provided if any are provided.", name);
            }
            ...
            object actionArgument, bool promote, bool neverConsiderToBeDuplicate, TimeSpan? durationOverride = null)
        {
            ...
            var snackbarMessageQueueItem = new SnackbarMessageQueueItem(
                content,
            ...
            if (promote)
                ...
                _ = _snackbarMessages.AddLast(snackbarMessageQueueItem);

            _ = _messageWaitingEvent.Set();
        }
        ...
        private void InsertAsLastNotPromotedNode(SnackbarMessageQueueItem snackbarMessageQueueItem)
        {
            ...
            while (node != null)
            {
                ...
                {
                    _ = _snackbarMessages.AddBefore(node, snackbarMessageQueueItem);
                    return;
                }
                node = node.Next;
            }
        ...
        private async void PumpAsync()
        {
            while (!_isDisposed)
            {
                ...
                if (exemplar == null)
                {
                    ...
                    _ = _disposedEvent.WaitOne(TimeSpan.FromSeconds(1));
                    continue;
                }
        ...
        {
            return dispatcher.InvokeAsync(() => _pairedSnackbars.FirstOrDefault(sb =>
            {
                if (!sb.IsLoaded || sb.Visibility != Visibility.Visible)
                    return false;

                return Window.GetWindow(sb)?.WindowState != WindowState.Minimized;
            }));
        }
        ...
        private async Task ShowAsync(Snackbar snackbar, SnackbarMessageQueueItem messageQueueItem)
        {
            await Task.Run(async () =>
            {
                ...
                var mouseNotOverManagedWaitHandle = await snackbar.Dispatcher
                    .InvokeAsync(() => CreateAndShowMessage(snackbar, messageQueueItem, actionClickWaitHandle));

                var durationPassedWaitHandle = new ManualResetEvent(false);

                _ = DurationMonitor.Start(
                    messageQueueItem.Duration.Add(snackbar.ActivateStoryboardDuration),
                    _pausedEvent,
                ...
                await snackbar.Dispatcher.InvokeAsync(() => snackbar.SetCurrentValue(Snackbar.IsActiveProperty, false));

                //we could wait for the animation event, but just doing
                //this for now...at least it is prevent extra call back hell
                _ = _disposedEvent.WaitOne(snackbar.DeactivateStoryboardDuration);

                //remove message on snackbar
                await snackbar.Dispatcher.InvokeAsync(() => snackbar.SetCurrentValue(Snackbar.MessageProperty, null));
        ...
        private static MouseNotOverManagedWaitHandle CreateAndShowMessage(
            UIElement snackbar,
            SnackbarMessageQueueItem messageQueueItem,
